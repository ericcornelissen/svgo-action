...
const svgoV1Mock = jest.fn()
  .mockReturnValue({ })
  .mockName("SVGO v1 constructor");
const svgoV2Mock = {
  optimize: jest.fn()
    .mockReturnValue({ })
...
jest.mock("svgo-v1", () => svgoV1Mock);
jest.mock("svgo-v2", () => svgoV2Mock);

import { SVGOptimizer } from "../../../src/svgo/svgo-wrapper";

describe("svgo/svgo-wrapper.ts", () => {
  ...
  describe("SVGOptimizer with SVGO v1", () => {
    describe("::optimize", () => {
      let optimizeMock: jest.Mock<Promise<{ data: string }>, [string]>;

      beforeAll(() => {
        optimizeMock = jest.fn()
          .mockResolvedValue({ data: null })
          .mockName("svgo.optimize");

        svgoV1Mock.mockReturnValueOnce({
      ...
      test("valid SVG", async () => {
        ...
        const svgo = new SVGOptimizer(1);

        const [result, err] = await svgo.optimize(validSvg);
        expect(err).toBeNull();
      ...
      test("invalid SVG", async () => {
        when(optimizeMock)
          .calledWith(validSvg)
          .mockRejectedValueOnce(new Error("optimize error"));

        const svgo = new SVGOptimizer(1);

        const [, err] = await svgo.optimize(invalidSvg);
        expect(err).not.toBeNull();
      });
    });
  });

  describe("SVGOptimizer with SVGO v2", () => {
    let svgo;

    beforeEach(() => {
      svgo = new SVGOptimizer(2);
    });

    describe("::optimize", () => {
      test("valid SVG", async () => {
        when(svgoV2Mock.optimize)
          .calledWith(validSvg, expect.anything())
          .mockReturnValueOnce({ data: optimizedSvg });
      ...
      test("optimization error", async () => {
        ...
        const [, err] = await svgo.optimize(validSvg);
        expect(err).not.toBeNull();
      });
    });
  });
